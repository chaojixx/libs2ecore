...
void S2EExecutor::registerCpu(S2EExecutionState *initialState, CPUArchState *cpuEnv) {
    ...
    MemoryObject *symbolicRegs = addExternalObject(*initialState, cpuEnv, offsetof(CPUArchState, eip),
                                                   /* isReadOnly = */ false,
                                                   /* isUserSpecified = */ false,
                                                   /* isSharedConcrete = */ false);

    /* Add the rest of the structure as concrete-only area */
    MemoryObject *concreteRegs = addExternalObject(*initialState, ((uint8_t *) cpuEnv) + offsetof(CPUArchState, eip),
...
#elif defined(TARGET_ARM)
    /* Add registers and eflags area as a true symbolic area */
    MemoryObject *symbolicRegs = addExternalObject(*initialState, cpuEnv, offsetof(CPUArchState, regs[15]),
                                                   /* isReadOnly = */ false,
                                                   /* isUserSpecified = */ false,
                                                   /* isSharedConcrete = */ false);

    /* Add the rest of the structure as concrete-only area */
    MemoryObject *concreteRegs = addExternalObject(*initialState, ((uint8_t *) cpuEnv) + offsetof(CPUArchState, regs[15]),
