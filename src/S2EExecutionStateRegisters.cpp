...
CPUArchState *S2EExecutionStateRegisters::getCpuState() const {
    CPUArchState *cpu = *m_active
                           ? (CPUArchState *) (s_concreteRegs->address - offsetof(CPUArchState, eip))
                           : (CPUArchState *) (m_concreteRegs->getConcreteStore(true) - offsetof(CPUArchState, eip));
...
CPUArchState *S2EExecutionStateRegisters::getCpuState() const {
    CPUArchState *cpu = *m_active
                           ? (CPUArchState *) (s_concreteRegs->address - offsetof(CPUArchState, regs[15]))
                           : (CPUArchState *) (m_concreteRegs->getConcreteStore(true) - offsetof(CPUArchState, regs[15]));
...
void S2EExecutionStateRegisters::readConcreteRegion(unsigned offset, void *buffer, unsigned size) const {
    ...
    assert((width == 1 || (width & 7) == 0) && width <= 64);
    assert(offset >= offsetof(CPUArchState, eip));
...
bool S2EExecutionStateRegisters::getRegionType(unsigned offset, unsigned size, bool *isConcrete) {
    if (offset + size <= offsetof(CPUArchState, eip)) {
        *isConcrete = false;
        return true;
    } else if (offset >= offsetof(CPUArchState, eip)) {
        *isConcrete = true;
        return true;
    } else {
        return false;
...
void S2EExecutionStateRegisters::readConcreteRegion(unsigned offset, void *buffer, unsigned size) const {
    unsigned width = size * 8;
    assert((width == 1 || (width & 7) == 0) && width <= 64);
    assert(offset >= offsetof(CPUArchState, regs[15]));
    assert(offset + Expr::getMinBytesForWidth(width) <= sizeof(CPUArchState));
...
bool S2EExecutionStateRegisters::getRegionType(unsigned offset, unsigned size, bool *isConcrete) {
    if (offset + size <= offsetof(CPUArchState, regs[15])) {
        *isConcrete = false;
        return true;
    } else if (offset >= offsetof(CPUArchState, regs[15])) {
        *isConcrete = true;
        return true;
    } else {
        return false;
